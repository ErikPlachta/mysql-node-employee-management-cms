...
const getEmployee = async function(id) {
    ...
    if(id != undefined) {
      const response = await fetch(`http://127.0.0.1:${process.env.MY_PORT}/api/employees/${id}`)
...
const getEmployees = async function() {
  ...
  const response = await fetch(`http://127.0.0.1:${process.env.MY_PORT}/api/employees/`)
    .then(response => response.json())
    .then(json => {
      return JSON.stringify(json);
    })
